...
std::mutex reverse_response_mtx;
std::vector<int> reverse_response_order;

asio::awaitable<void> delayed_response(rpc_context ctx, int value,
                                       std::chrono::milliseconds delay,
                                       bool remember_order = false) {
  asio::steady_timer timer(co_await asio::this_coro::executor);
  timer.expires_after(delay);
  co_await timer.async_wait(asio::use_awaitable);

  if (remember_order) {
    std::scoped_lock lock(reverse_response_mtx);
    reverse_response_order.push_back(value);
  }
  co_await ctx.response(value);
}

int reverse_delay_response(int value) {
  rpc_context ctx;
  auto executor = ctx.get_executor();
  asio::co_spawn(executor,
                 delayed_response(std::move(ctx), value,
                                  std::chrono::milliseconds((4 - value) * 100),
                                  true),
                 asio::detached);
  return 0;
}

int timeout_delay_response(int value) {
  rpc_context ctx;
  auto executor = ctx.get_executor();
...
int never_response(int value) {
  rpc_context ctx;
  return value;
}

char echo_char(char value) { return value; }
...
asio::awaitable<void> test_multiplex_out_of_order(rpc_client &client) {
  {
    std::scoped_lock lock(reverse_response_mtx);
    reverse_response_order.clear();
  }

  rpc_server server("127.0.0.1:9011");
  server.register_handler<reverse_delay_response>();
  server.register_handler<echo_char>();
  CHECK(!server.async_start());

  auto ec = co_await client.connect("127.0.0.1:9011");
  CHECK(!ec);

  auto [r1, r2, r3] = co_await (
      client.call_for<reverse_delay_response>(std::chrono::seconds(2), 1) &&
      client.call_for<reverse_delay_response>(std::chrono::seconds(2), 2) &&
      client.call_for<reverse_delay_response>(std::chrono::seconds(2), 3));
  CHECK(r1.ec == rpc_errc::ok);
  CHECK(r2.ec == rpc_errc::ok);
  CHECK(r3.ec == rpc_errc::ok);
  CHECK(r1.value == 1);
  CHECK(r2.value == 2);
  CHECK(r3.value == 3);

  char input = 'A';
  auto char_result = co_await client.call<echo_char>(input);
  CHECK(char_result.ec == rpc_errc::ok);
  CHECK(char_result.value == input);
  {
    std::scoped_lock lock(reverse_response_mtx);
    CHECK(reverse_response_order == (std::vector<int>{3, 2, 1}));
  }
  server.stop();
  co_return;
}

TEST_CASE("test multiplex out of order response") {
  rpc_client client;
  sync_wait(client.get_executor(), test_multiplex_out_of_order(client));
}

asio::awaitable<void> test_timeout_response(rpc_client &client) {
  rpc_server server("127.0.0.1:9012");
  server.register_handler<timeout_delay_response>();
  CHECK(!server.async_start());
  auto ec = co_await client.connect("127.0.0.1:9012");
  CHECK(!ec);

  auto timeout_result = co_await client.call_for<timeout_delay_response>(
      std::chrono::milliseconds(20), 1);
  CHECK(timeout_result.ec == rpc_errc::request_timeout);

  auto next_result = co_await client.call<timeout_delay_response>(2);
  ...
  auto final_result = co_await client.call<timeout_delay_response>(3);
  CHECK(final_result.ec == rpc_errc::ok);
  CHECK(final_result.value == 3);
  server.stop();
}

TEST_CASE("test timeout response does not poison next call") {
  rpc_client client;
...
asio::awaitable<void>
test_client_destruction(std::unique_ptr<rpc_client> &client) {
  rpc_server server("127.0.0.1:9013");
  server.register_handler<never_response>();
  CHECK(!server.async_start());
  auto ec = co_await client->connect("127.0.0.1:9013");
  CHECK(!ec);

  auto call1 = client->call<never_response>(1);
  auto call2 = client->call<never_response>(2);
...
TEST_CASE("test client destruction completes all pending calls") {
  auto client = std::make_unique<rpc_client>();
  auto executor = client->get_executor();
  sync_wait(executor, test_client_destruction(client));
}

TEST_CASE("test single char codec") {
  auto data = rpc_codec::pack_args('A');
  CHECK(rpc_codec::unpack<char>(data) == 'A');
}
